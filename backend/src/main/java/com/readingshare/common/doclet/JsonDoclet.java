...
public class JsonDoclet implements Doclet {
    ...
    public boolean run(DocletEnvironment environment) {
        try {
            ...
            documentation.put("classes", classes.stream()
                    .sorted((a, b) -> a.getQualifiedName().toString().compareTo(b.getQualifiedName().toString()))
    ...
    private Map<String, Object> processClass(TypeElement clazz) {
        ...
        // Prepare auto-generated method names for records and enums
        Set<String> autoMethodNames = new HashSet<>();
        if (clazz.getKind() == ElementKind.RECORD) {
            // record components generate accessor methods
            autoMethodNames.addAll(clazz.getRecordComponents().stream()
                    .map(RecordComponentElement::getSimpleName)
                    .map(Object::toString)
                    .collect(Collectors.toSet()));
            autoMethodNames.add("equals");
            autoMethodNames.add("hashCode");
            autoMethodNames.add("toString");
        } else if (clazz.getKind() == ElementKind.ENUM) {
            // enum generates values() and valueOf(String) methods
            autoMethodNames.add("values");
            autoMethodNames.add("valueOf");
        }
        List<Map<String, Object>> methods = new ArrayList<>();
        for (ExecutableElement method : ElementFilter.methodsIn(clazz.getEnclosedElements())) {
            String methodName = method.getSimpleName().toString();
            DocCommentTree methodTree = docTrees.getDocCommentTree(method);
            boolean isAutoGen = autoMethodNames.contains(methodName) && methodTree == null;
            Map<String, Object> methodMap = processMethod(method);
            if (isAutoGen) {
                continue; // Skip auto-generated methods
                // methodMap.put("description", "Java Compiler が自動的に追加したメソッド。"); //
                // auto-generated by javac
                // @SuppressWarnings("unchecked")
                // List<Map<String, Object>> params = (List<Map<String, Object>>)
                // methodMap.get("params");
                // for (Map<String, Object> param : params) {
                // param.put("description", "暗黙に生成された"); // auto-generated
                // }
                // if (methodMap.containsKey("return")) {
                // @SuppressWarnings("unchecked")
                // Map<String, Object> returnInfo = (Map<String, Object>)
                // methodMap.get("return");
                // returnInfo.put("description", "暗黙に生成された"); // auto-generated
                // }
            }
