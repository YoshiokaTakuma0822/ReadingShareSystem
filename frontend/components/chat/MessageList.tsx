...
const MessageItem = ({ data, index, style }: {
    ...
    const senderName = data.members[message.senderId]?.name || '不明なユーザー'

    const renderMessage = (message: MessageResponse, senderName: string) => {
        switch (message.type) {
            case "JOIN":
                return `${senderName}さんが参加しました`
            case "LEAVE":
                return `${senderName}さんが退出しました`
            default:
            ...
            >
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <p className="mt-1">{renderMessage(message, senderName)}</p>
                    </div>
                    {(() => {
                        const date = new Date(
...
export function MessageList({ loadTriggerCount = 1 }: { loadTriggerCount?: number }) {
    const { messages, loadOlderMessages, currentUser, members } = useChatContext()
    ...
    // Handle items rendered to determine if at bottom
    const handleItemsRendered = useCallback(({ visibleStartIndex, visibleStopIndex }: { visibleStartIndex: number, visibleStopIndex: number }) => {
        ...
        // 最後のアイテムが表示されているかどうかで最下部判定
        const atBottom = visibleStopIndex >= msgs.length - 1
    ...
    const prevMessagesRef = useRef<MessageResponse[]>([])
    ...
    useEffect(() => {
        ...
        // 自分メッセージと他者メッセージを分離
        const ownMessages = newMessages.filter(m => m.senderId === currentUser?.id)
        const otherMessages = newMessages.filter(m => m.senderId !== currentUser?.id)
